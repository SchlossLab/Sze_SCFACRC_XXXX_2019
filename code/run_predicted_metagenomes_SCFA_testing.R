...
combined_data_hl <- sapply(scfas, 
                  function(x) make_rf_data(x, nzv_full_pi_data, all_scfa_data, 
                                            c(metaF$initial, metaF$followUp), "high_low"), simplify = F)
...
for(j in 1:100){
  ...
  rf_models <- sapply(scfas, 
                      function(x) make_rf_model(x, j, "training_data", "high_low", "rf", "ROC", final_rf_data),
                      simplify = F)
  ...
  rf_test_data <- sapply(scfas, 
                         function(x) 
                           run_prediction(x, rf_models, "test_data", "raw", "high_low", final_rf_data), 
                         simplify = F)
  
  # Generate the ROC curves
  test_roc <- sapply(scfas, 
                     function(x) get_test_roc(x, "high_low", "tempPredictions", rf_test_data), simplify = F)
