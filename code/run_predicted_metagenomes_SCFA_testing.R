...
make_rf_data <- function(scfa_name, dataTable, scfa_data, exclude_samples){
...
combined_data <- sapply(scfas, 
                  function(x) make_rf_data(x, nzv_full_pi_data, scfa_data, 
                                           c(metaF$initial, metaF$followUp)), simplify = F)
