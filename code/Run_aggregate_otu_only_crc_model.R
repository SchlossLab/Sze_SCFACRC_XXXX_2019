...
### and pull out most important OTUs used within it (Carcinoma OTU only Models)
## Marc Sze
...
loadLibs(c("tidyverse", "scales", "caret", "pROC"))
...
best_model_data <- as.data.frame(matrix(nrow = n, ncol = 8))

for(i in 1:n){
  
  load(paste("exploratory/crc_OTU_only_RF_model_", i, ".RData", sep=""))
  ...
  pred_roc[[paste("run_", i, sep = "")]] <- 
    roc(test_data_list[[paste("run_", i, sep="")]]$dx ~ 
...
write.csv(
  ...
  "data/process/tables/crc_otu_only_AUC_model_summary.csv", row.names = F)
...
#          "data/process/tables/adn_test_data_roc.csv", row.names = F)
...
tax_df <- tax_df %>% 
  mutate(otu = as.character(rownames(.)), 
         Genus = gsub("\\(\\d{2}\\d?\\)", "", Genus), 
         Genus = str_replace_all(Genus, "_unclassified", "")) %>% 
  select(otu, Genus)

summary_otu_mda <- summary_otu_mda %>% left_join(tax_df, by = c("Variable" = "otu"))


# Write out the raw information for the importantance by MDA to a table
write.csv(raw_mda_otu_data, 
          "data/process/tables/crc_otu_only_raw_mda_values.csv", row.names = F)


# Write out the median and IQR for the MDA of the most important variables

write.csv(summary_otu_mda, 
          "data/process/tables/crc_otu_only_MDA_Summary.csv", row.names = F)
