...
# Set up mapping file name
map_name <- "scfa_plate_metadata.csv"

# Set up raw text file path
raw_path <- "data/raw/scfa/"

# Set up output file path
output_path <- "data/scfa/"
...
get_scfa_data <- function(scfa_name, uniq_name, ending, path_to_file){
  ...
  if(!is.null(scfa_name)){
    ...
    tempData <- read.delim(
      ...
      skip = 2, header = T, stringsAsFactors = F, row.names = 1, na="-----") %>%
      select(Data.Filename, Sample.ID, Sample.Name, Ret..Time,
             Area, Peak.Start, Peak.End, Conc.) %>%
      mutate(
        Area = ifelse(is.na(Area), invisible(0), invisible(Area)),
...
perform_peak_check <- function(dataTable){
  ...
  tempStds <- dataTable %>% drop_na() %>% filter(grepl("mM", Sample.Name) == TRUE)
...
make_standard_table <- function(peak_check_data){
  ...
  tempData <- peak_check_data %>%
    ...
    separate(Sample.Name, c("conc", "unit", "mix"), sep = " ") %>%
...
write_the_data <- function(scfa_of_interest, dataList,
                           output_path, plate, final_name){
print(paste(output_path, final_name, "_",
			plate, "_", scfa_of_interest, ".csv", sep = ""))
  write.csv(dataList[[scfa_of_interest]],
            paste(output_path, final_name, "_",
...
transform_data <- function(scfas, plate, stds_positions, sample_positions){

	test <- sapply(scfas,
	               function(x)
	                 get_scfa_data(x, plate, paste(x, ".txt", sep = ""), raw_path),simplify = F)
...
transform_data(scfas, "plate1_scfa_crc",
							list(std1 = c(1:9), std2 = c(66:74)),
							list(g1 = c(11:65)))

transform_data(scfas, "plate2_scfa_crc",
							list(std1 = c(1:9), std2 = c(61:69)),
							list(g1 = c(11:60)))

transform_data(scfas, "plate3_scfa_crc",
							list(std1 = c(1:9), std2 = c(60:68), std3 = c(118:126)),
							list(g1 = c(11:58), g2 = c(69:116)))

transform_data(scfas, "plate4_scfa_crc",
							list(std1 = c(1:9), std2 = c(60:68), std3 = c(118:126)),
							list(g1 = c(11:58), g2 = c(69:116)))

transform_data(scfas, "plate5_scfa_crc",
							list(std1 = c(1:9), std2 = c(60:68), std3 = c(118:126)),
							list(g1 = c(11:58), g2 = c(69:116)))

transform_data(scfas, "plate6_scfa_crc",
							list(std1 = c(1:9), std2 = c(60:68), std3 = c(118:126)),
							list(g1 = c(11:58), g2 = c(69:116)))

transform_data(scfas, "plate7_scfa_crc",
							list(std1 = c(1:9), std2 = c(68:76), std3 = c(126:134)),
							list(g1 = c(19:66), g2 = c(77:124)))

transform_data(scfas, "plate8_scfa_crc",
							list(std1 = c(1:9), std2 = c(76:84), std3 = c(130:138)),
							list(g1 = c(31:74), g2 = c(85:128)))
