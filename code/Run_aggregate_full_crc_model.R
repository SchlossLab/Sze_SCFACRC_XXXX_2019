...
### and pull out most important OTUs used within it (Carcinoma Full Models)
## Marc Sze
...
for(i in 1:n){
  ...
  load(paste("exploratory/crc_full_RF_model_", i, ".RData", sep=""))
...
write.csv(
  ...
  "data/process/tables/crc_full_AUC_model_summary.csv", row.names = F)
...
write.csv(raw_mda_otu_data, 
          "data/process/tables/crc_full_raw_mda_values.csv", row.names = F)


# Write out the median and IQR for the MDA of the most important variables

write.csv(summary_otu_mda, 
          "data/process/tables/crc_full_MDA_Summary.csv", row.names = F)
