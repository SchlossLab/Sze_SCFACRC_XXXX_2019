...
### and pull out most important OTUs used within it (Adenoma OTU only Models)
## Marc Sze
...
for(i in 1:n){
  ...
  load(paste("exploratory/adn_OTU_Only_RF_model_", i, ".RData", sep=""))
...
write.csv(
  ...
  "data/process/tables/adn_otu_only_AUC_model_summary.csv", row.names = F)
...
write.csv(raw_mda_otu_data, 
          "data/process/tables/adn_otu_only_raw_mda_values.csv", row.names = F)


# Write out the median and IQR for the MDA of the most important variables

write.csv(summary_otu_mda, 
          "data/process/tables/adn_otu_only_MDA_Summary.csv", row.names = F)
