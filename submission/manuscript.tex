...
for hyperparameter selection and training the model, and the test set
was used for evaluating prediction performance. For each model, the best
...
carcinomas (N=120). (B) A subset of individuals diagnosed with adenomas
(N=41) or carcinomas (N=26) who underwent treatment were resampled a
year after the initial sampling; one extreme propionate value (124.4
mmol/kg) was included in the adenoma analysis but censored from the
...
differentiate between subjects with normal colons and those with
...
the data and a testing set with the remaining samples. The training data
was used to fit the mtry hyperparameter using 100 five-fold cross
validations for six values of mtry. The mtry with the largest AUROC was
selected to train the model, which was then tested on the held out 20\%
of the samples.
...
concentration of each SCFA using each subjects' microbiome data
...
remaining samples. The training data was used to fit the mtry
hyperparameter using 100 five-fold cross validations for six values of
mtry. The mtry with the largest R\textsuperscript{2} was selected to
train the model, which was then tested on the held out 20\% of the
samples.
